...
#
# システムを起動させ，トレーニングあるいはテストの実行を行います．
# コマンドライン引数を指定することにより，詳細設定が行えます．
...
def parse_args():
    ...
    parser.add_argument('--img-size', type=int, default=100, help='Image size for input to network (default: 100)')
...
def main():
    ...
    if args.mode == 'train':
        ...
        model = models.FRModel(param)
        _ = model.test('choose', args.test_img_path, args.num_recom)
